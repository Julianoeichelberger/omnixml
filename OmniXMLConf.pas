...
{$ENDIF}
  ...
  if (FxmlDoc <> nil) and (FxmlRoot <> nil) then
  begin
    if FxmlDoc.DocumentElement.TagName <> 'conf' then
    begin
      FxmlDoc := CreateXMLDoc;
      FxmlDoc.PreserveWhiteSpace := False;
      FxmlRoot := nil;
...
begin
  ...
  begin
    ...
    if (Control as TForm).BorderStyle = bsSizeable then
    begin
